...
const partyBgVariants: Variants = {
  idle: {
    background: 'linear-gradient(to right, #1a0e0a, #0f0f11, #061e2d)',
    scale: 1,
    transition: {
      duration: 1,
    },
  ...
  party: {
    ...
    transition: {
      ...
      ease: 'easeInOut',
      repeat: Infinity,
      repeatType: 'loop' as const, // 👈 KEY FIX
    },
  },
};
...
const Hero = () => {
  ...
  // Optional: control animation imperatively if needed
  const controls = useAnimation();

  useEffect(() => {
    if (isPlaying) {
      controls.start('party');
    } else {
      controls.start('idle');
    }
  }, [isPlaying, controls]);
        ...
        >
          ...
          <h2 className="text-xl sm:text-2xl md:text-3xl lg:text-5xl font-semibold mb-6">
            Full Stack Developer
                  ...
                  {/* Catch My Vibe Button - MOBILE VIEW */}
  <motion.div className="block lg:hidden mb-6">
    <Button
      onClick={() => setShowVibe(true)}
      variant="hero"
      size="sm"  // smaller button size on mobile
      className="bg-gradient-to-r from-purple-600 to-indigo-500 text-white px-3 py-1 rounded-md shadow-lg hover:scale-105 transition-transform"
    >
      Catch My Vibe
